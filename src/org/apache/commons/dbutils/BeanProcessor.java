...
public class BeanProcessor {
    ...
    public <T> T toBean(ResultSet rs, Class<T> type) throws SQLException {
        ...
        // SHEN.GANG modify: add 'type' param to mapColumnsToProperties
    ...
    private <T> T createBean(ResultSet rs, Class<T> type,
        ...
        for (int i = 1; i < columnToProperty.length; i++) {
            ...
            /* SHEN.GANG comment at 2014-04-25
            if (propType != null && value == null && propType.isPrimitive()) {
                value = primitiveDefaults.get(propType);
            }*/
            
            // SHEN.GANG add at 2014-04-25
            if (propType != null && value == null ) {
            	if( propType.isPrimitive() ){
            		value = primitiveDefaults.get(propType);
            	}else if( "java.lang.String".equals( propType.getName() ) ){
            		value = "";
            	}
            }
    ...
    protected int[] mapColumnsToProperties(ResultSetMetaData rsmd,
            PropertyDescriptor[] props, Class<?> beanType) throws SQLException {

        int cols = rsmd.getColumnCount();
        int columnToProperty[] = new int[cols + 1];
        Arrays.fill(columnToProperty, PROPERTY_NOT_FOUND);
        
        // SHEN.GANG add to work with fastDB-Annotation at 2014/02/07
        Map<String, String> ann_mapping = PersistenceUtil.getFieldAnnotationMapping(beanType);
        ...
        for (int col = 1; col <= cols; col++) {
            ...
            for (int i = 0; i < props.length; i++) {
            	// SHEN.GANG modify to work with fastDB-Annotation at 2014/02/07
                //if (columnName.equalsIgnoreCase(props[i].getName())) {
            	if ( columnName.equalsIgnoreCase(props[i].getName()) 
            			|| (null != ann_mapping.get(columnName) && ann_mapping.get(columnName).equalsIgnoreCase(props[i].getName()))) {
